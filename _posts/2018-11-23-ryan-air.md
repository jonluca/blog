...
image: "/images/ryanair-flight-prices.png"
---
...
<link
  rel="stylesheet"
  type="text/css"
  href="{{ site.url }}/stylesheets/custom-dropdown.css"
/>
...
{% include image.html footnote="Ryanair flight prices" file="ryanair-flight-prices" %}
...
{% include image.html footnote="Ryanair filtered prices" file="ryanair-filtered-flight" %}
...
{% include image.html footnote="Ryanair filtered HTML" file="ryanair-filtered-html" %}
...
The best part of this is that _all_ the above airports fly to `TSF` - we can add many more airports to our trip this way. We know that any path in the graph above lets us return to `TSF`, which means we can do a trip with `N` many distinct stops, where N is the length of the longest non repeating path[^1]!
