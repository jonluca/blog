---
title: "SPF, DKIM and DMARC out in the wild"
...
Email authentication has had a turbulent history. SMTP did not have a native form of authentication. This was not a problem in the 1980s because there were simply too few people using it - the only people were universities and corporations actively involved in building the internet. Since then there have been a few attempts at solving the problem of verifying the email has not been tampered with and that it is coming from a trusted source; these have all behaved as bandaids, though, and email spam and phishing remain a huge problem.

I wanted to explore the actual usage of these authentication methods by the most popular sites and companies in the world - specifically, the Alex top 100 domains and the Fortune 500 companies.
...
This says that it should pass any IP in either `66.3.159.0/24` range, include all valid domains for `mail.zendesk.com` and `_spf.google.com` and fail all others.
